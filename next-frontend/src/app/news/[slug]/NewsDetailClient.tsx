...
function NewsVideoPlayer({ videoUrl, title, resourceId, activeVideoId, onVideoPlay }: { videoUrl: string; title: string; resourceId: number; activeVideoId: number | null; onVideoPlay: (id: number) => void }) {
...
export default function NewsDetailClient({ news, resources }: { news: News; resources: NewsResource[] }) {
  ...
  const allImages = [
    ...(news.image_url || news.feature_image ? [{
      type: 'featured',
      url: news.image_url || news.feature_image || '',
      caption: news.title
    }] : []),
  ...
  return (
    <section className="py-12 md:py-16 bg-gray-50 font-Roboto">
      <div className="max-w-4xl mx-auto px-4">
        ...
        >
          <span className="mr-2">←</span> Back to all news
        </Link>
        ...
        <article className="bg-white rounded-3xl shadow-lg overflow-hidden">
          ...
          {news.image_url || news.feature_image ? (
            ...
            >
              <img
                src={getFullImageUrl(news.image_url || news.feature_image || "")}
                alt={news.title}
          ...
          <div className="p-8 md:p-12">
            ...
            <p className="text-gray-500 text-sm mb-6">
              {news.news_date || news.date || news.created_at 
                ? new Date(news.news_date || news.date || news.created_at || '').toLocaleDateString('en-US', { 
                    year: 'numeric', 
                    month: 'long', 
                    day: 'numeric' 
                  })
            ...
            {imageResources.length > 0 && (
              <div className="mb-12">
                <h2 className="text-2xl font-bold text-[#191938] mb-6">Related Images</h2>
                  ...
                  {imageResources.map((resource, idx) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        const imageIndex = (news.image_url || news.feature_image ? 1 : 0) + idx;
                        setSelectedImageIndex(imageIndex);
                      }}
                      className="rounded-lg overflow-hidden bg-gray-200 hover:opacity-85 transition cursor-pointer text-left"
                    >
                      <img
                      ...
                      />
                      {resource.caption && (
                        <p className="p-3 text-sm text-gray-700 bg-white">
                          {resource.caption}
                        </p>
                      )}
                    </button>
            ...
            {videoResources.length > 0 && (
              <div className="mb-12">
                <h2 className="text-2xl font-bold text-[#191938] mb-6">Related Videos</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            >
              ...
              {allImages[selectedImageIndex].caption && (
                <div className="text-center py-2 border-t border-white/10">
                  <p className="text-white/80 text-sm">{allImages[selectedImageIndex].caption}</p>
                </div>
              )}
